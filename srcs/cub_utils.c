...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/20 17:56:05 by akhachat          #+#    #+#             */
/*   Updated: 2021/05/20 17:56:14 by akhachat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_colors(t_norm_print_map *n, t_cub *cub, int *x, int *y)
{
	if (*y < n->draw_start)
		...
		my_rgb(cub->p.ceiling_r, cub->p.ceiling_g, cub->p.ceiling_b));
	if (*y >= n->draw_start && *y <= n->draw_end)
		attach_textures(n, cub, x, y);
	if (*y > n->draw_end && *y < cub->p.res_l)
		my_mlx_pixel_put(&cub->data, *x, *y,
		my_rgb(cub->p.floor_r, cub->p.floor_g, cub->p.floor_b));
}
